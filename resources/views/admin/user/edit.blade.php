...
<div class="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto">
    ...
    @php
        $foto = $user->karyawan && $user->karyawan->foto_profil
            ? asset('storage/images/foto_profil/' . $user->karyawan->foto_profil)
    ...
    <form action="{{ route('admin.user.update', $user->id_user) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        @if (old('role', $user->role ) === 'karyawan')
            {{-- Karyawan Fields --}}
            <div id="karyawanFields" class="space-y-4 {{ old('role', $user->role) === 'karyawan' ? '' : 'hidden' }}">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <img id="previewFoto" src="{{ $foto }}" alt="Preview Foto" class="w-32 h-32 object-cover rounded border {{ optional($user->karyawan)->foto_profil ? '' : 'hidden' }}">
                    </div>
                </div>
            </div>
        @else
            <div id="karyawanFields" class="space-y-4 hidden"></div>
        @endif
